...
MORDOR_CONF_SRC="$SRC_DIR/btrbk_mordor.conf"
SERVICE_SRC="$SRC_DIR/btrbk_shire.service"
TIMER_SRC="$SRC_DIR/btrbk_shire.timer"
SERVICE_MORDOR_SRC="$SRC_DIR/btrbk_mordor.service"
TIMER_MORDOR_SRC="$SRC_DIR/btrbk_mordor.timer"

DEST_CONF="/etc/btrbk/btrbk_shire.conf"
DEST_MORDOR_CONF="/etc/btrbk/btrbk_mordor.conf"
...
- copy the shire and mordor configs to /etc/btrbk
- copy service and timer to $SYSTEMD_DIR (for both shire and mordor)
...
for f in "$CONF_SRC" "$MORDOR_CONF_SRC" "$SERVICE_SRC" "$TIMER_SRC" "$SERVICE_MORDOR_SRC" "$TIMER_MORDOR_SRC"; do
	if [[ ! -f "$f" ]]; then
...
run_cmd sudo cp -v "$MORDOR_CONF_SRC" "$DEST_MORDOR_CONF"
run_cmd sudo chown root:root "$DEST_MORDOR_CONF"
run_cmd sudo chmod 644 "$DEST_MORDOR_CONF"
...
# Install systemd unit files (mordor)
run_cmd sudo cp -v "$SERVICE_MORDOR_SRC" "$SYSTEMD_DIR/"
run_cmd sudo cp -v "$TIMER_MORDOR_SRC" "$SYSTEMD_DIR/"
run_cmd sudo chown root:root "$SYSTEMD_DIR/$(basename "$SERVICE_MORDOR_SRC")" "$SYSTEMD_DIR/$(basename "$TIMER_MORDOR_SRC")"
run_cmd sudo chmod 644 "$SYSTEMD_DIR/$(basename "$SERVICE_MORDOR_SRC")" "$SYSTEMD_DIR/$(basename "$TIMER_MORDOR_SRC")"

# Reload systemd and enable/start timers
run_cmd sudo systemctl daemon-reload
run_cmd sudo systemctl enable --now btrbk_shire.timer
run_cmd sudo systemctl enable --now btrbk_mordor.timer

# Start the services we installed (they run configured jobs)
if command -v systemctl >/dev/null 2>&1; then
	if $DRY_RUN; then
		echo "+ sudo systemctl start btrbk_shire.service"
		echo "+ sudo systemctl start btrbk_mordor.service"
	else
		...
		echo "Starting btrbk_mordor.service..."
		if sudo systemctl start btrbk_mordor.service 2>&1 | tee /tmp/btrbk_mordor_service_start.log; then
			echo "Mordor service started (log: /tmp/btrbk_mordor_service_start.log)"
		else
			echo "Note: starting mordor service returned non-zero. See /tmp/btrbk_mordor_service_start.log" >&2
		fi
	fi
else
